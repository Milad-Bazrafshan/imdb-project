...
public class PrincipalsService {
    ...
    public Page<TitlePrincipals> getTitlePrincipals(ImdbRatingFilterDto filter) {
        return titlePrincipalsRepository.findAll(Specification.allOf(getImdbRatingSpec(filter)), PageRequest.of(filter.getPageNumber(), filter.getPageSize()));
    }

    private List<Specification<TitlePrincipals>> getImdbRatingSpec(ImdbRatingFilterDto filter) {
